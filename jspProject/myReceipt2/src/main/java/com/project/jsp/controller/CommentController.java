...
public class CommentController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int result = service.insert(postId, memberId,content, parentCommentId);

	    if (result > 0) {
	        resp.sendRedirect(req.getContextPath() + "/detail?id=" + postId);
	    } else {
	        resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "댓글 저장 실패");
	    }
