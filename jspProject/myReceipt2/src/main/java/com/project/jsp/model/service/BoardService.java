...
public class BoardService {
	...
	public boolean deletePost(int postId) {
		...
		try {
			conn = Mysql.getConnection();
			result = dao.deletePost(conn, postId);
		} catch (SQLException e) {
			e.printStackTrace();
		}finally {
			try {
	...
	public boolean updatePost(Post post) {
		...
		try {
			conn = Mysql.getConnection();
			isUpdated  = dao.updatePost(conn, post);
		} catch (SQLException e) {
			e.printStackTrace();
		}finally {
			try {
	...
	public int insert(int id, String title, String content) {
		Connection conn = null;
		int result = 0;
		BoardDao dao = new BoardDao();
		try {
			conn = Mysql.getConnection();
			result  = dao.insert(conn, id, title, content);
		} catch (SQLException e) {
			e.printStackTrace();
		}finally {
			try {
